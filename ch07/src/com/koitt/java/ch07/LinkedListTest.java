...
public class LinkedListTest {
	public static void main(String[] args) {
		...
		list.addLast(1);		
		list.add("java");		
		list.addLast(3.4); // 맨 뒤에 붙임	
		list.addFirst("algol");	// 맨앞에 붙게됨
		list.addFirst(0.87); // 그 맨앞에 붙게됨		
		list.add(3, null); // 3번째에 null값을 붙이고 나머지 값들은 arraycopy로 뒤로 밀림	
		list.addLast(5.8);		
		
		Iterator it = list.iterator(); // list에서 iterator 객체를 뽑아냄
		print(it);
		print(list.descendingIterator()); // 거꾸로 출력하기
